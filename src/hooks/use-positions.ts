...
export function usePositions(ref?: GridRefs) {
  ...
  const columnsPosition = generateColumnsPosition(
    cellRef,
    tableRef,
  )
  const rowsPosition = generateRowsPosition(
    cellRef,
    tableRef,
  )
  ...
  const getCellAxis = (pageX: number, pageY: number) => {
    const col = columnsPosition.findIndex(
      (col) =>
        pageX >= col.left &&
        pageX <= col.left + col.element.offsetWidth,
    )

    console.log(rowsPosition, 'rowsPosition')
    const row = rowsPosition.findIndex(
      (row) =>
        pageY >= row.top &&
        pageY <= row.top + row.element.offsetHeight,
    )
    return { col, row }
  }
